...
In Next.js 14.1.1, the `experimental.turbo.resolveExtensions` field in `next.config.js` was used to configure resolve extensions. As of Next.js 15.0, this field has been moved up one level to `experimental.resolveExtensions`, which now affects both Turbopack and Webpack configurations.
